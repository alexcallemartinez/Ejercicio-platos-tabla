const tipos = [
  ...
  const selecCategoria=document.getElementById("selectCategoria");
  ...
  const llenarCategoriasPorId = (idTipo)=>{
    if(idTipo ===0){
      selecCategoria.innerHTML="<option value='0'>-Seleccione Categoria-</option>";
      return;
    }
...
categoriasPorTipo.forEach((cat)=>{
  ...
  selecCategoria.appendChild(optionCat);
})
}
...
const load_tabla=()=>{
  ...
  /**
   * elemento.onchange se ejecuta cada vez que un input o select cambia su valor, es decir cuando se selecciona
   * un "option"
   */
